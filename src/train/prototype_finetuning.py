...
class PrototypeFinetuner:
    def __init__(self, device, k_shot=10, num_bg_prototypes=50):
        self.device = device
        self.k_shot = k_shot
    ...
    def _load_base_model(self):
        ...
        num_base_classes = 7 + 1
        in_features = model.roi_heads.box_predictor.cls_score.in_features
        model.roi_heads.box_predictor = FastRCNNPredictor(in_features, num_base_classes)
        
        model_path = 'saved_models/base_model.pth'
        if not os.path.exists(model_path):
            raise FileNotFoundError(f"Base model not found at {model_path}. Please run Part 1 first.")
    ...
    def create_prototypes(self, few_shot_data, all_image_paths):
        """Create initial object and background prototypes."""
        print("Creating initial object prototypes...")
        ...
        bg_features = []
        sampled_images = random.sample(all_image_paths, min(len(all_image_paths), 200))
        for img_path in tqdm(sampled_images, desc="Sampling backgrounds"):
            try:
                ...
                # Sample 5 random patch features from the feature map
                rand_h = torch.randint(0, h_feat, (5,))
                rand_w = torch.randint(0, w_feat, (5,))
    ...
    def finetune_prototypes(self, few_shot_data, num_epochs=50):
        ...
        print("Starting prototype finetuning...")
        self.all_prototypes = nn.Parameter(self.all_prototypes)
        
        optimizer = torch.optim.AdamW([self.all_prototypes], lr=1e-4, weight_decay=1e-3)
        ...
        for epoch in range(num_epochs):
            ...
            for annotation, class_name in tqdm(all_shots, desc=f"Epoch {epoch+1}/{num_epochs}"):
                class_idx = self.novel_class_to_idx[class_name]
                ...
                neg_indices = neg_indices[torch.randperm(len(neg_indices))[:32 - len(pos_indices)]]
    ...
    def save_prototypes(self, filename='saved_models/tuned_prototypes_20shot.pt'):
...
def run_finetuning():
    ...
    finetuner = PrototypeFinetuner(device, k_shot=10, num_bg_prototypes=50)
    
    few_shot_data, all_image_paths = finetuner.prepare_few_shot_data(root_dir=dataset_root)
    
    if not any(few_shot_data.values()):
        print("Could not find any data for the specified novel classes. Aborting.")
        return
        
    finetuner.create_prototypes(few_shot_data, all_image_paths)
    
    finetuner.finetune_prototypes(few_shot_data, num_epochs=50)
